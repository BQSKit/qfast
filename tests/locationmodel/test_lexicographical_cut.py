...
from qfast import lexicographical_cut


class TestLexicographicalCut ( tf.test.TestCase ):

    def check_lexicographical_cut_props ( self, num_qubits, gate_size ):
        locs = set( it.combinations( range( num_qubits ), gate_size ) )
        buckets = lexicographical_cut( locs )
    ...
    def test_lexicographical_cut ( self ):
        ...
        for num_qubits, gate_size in test_cases:
            self.check_lexicographical_cut_props( num_qubits, gate_size )
